...
genemap = readRDS("Data/Retrieved/Genes_and_diseases/Ranges/genemap.rds") 
genes.granges = readRDS("Data/Retrieved/Genes_and_diseases/Ranges/GenomicRanges.rds") 
GenesFrame = readRDS("Data/Retrieved/Genes_and_diseases/Ranges/human_genes_annotated.rds") 


genemap = readRDS("Data/Retrieved/Genes_and_diseases/Ranges/genemap_hg38_v115.rds") 
genes.granges = readRDS("Data/Retrieved/Genes_and_diseases/Ranges/GenomicRanges_hg38_v115.rds") 
GenesFrame = readRDS("Data/Retrieved/Genes_and_diseases/Ranges/human_genes_annotated_hg38_v115.rds") 



DiseasesCategories = read.csv("Data/Retrieved/Genes_and_diseases/Showcase/Ukb_DiseaseHierarchy.csv") 
...
GeneFrame_ARD_ARC = readRDS("Data/Retrieved/Genes_and_diseases/Diseases/GeneFrame_ARD_ARC.rds")

ImmGen = GeneFrame_ARD_ARC %>% dplyr::filter(ARC_Meaning %in% "immunological/systemic disorders") %>% dplyr::pull(Gene)


Ens = genemap %>% dplyr::filter(hgnc_symbol %in% ImmGen) %>% dplyr::pull(ensembl_gene_id)
Ens_New = genemap_new %>% dplyr::filter(hgnc_symbol %in% ImmGen) %>% dplyr::pull(ensembl_gene_id)

intersect(Ens,Ens_New)


genes.granges[ImmGen,]
genes.granges_new[ImmGen,]




GenesFrame %>% dplyr::filter(hgnc_symbol %in% ImmGen) %>% dplyr::arrange(hgnc_symbol) %>% tail()
GenesFrame_new %>% dplyr::filter(Gene %in% ImmGen) %>% dplyr::arrange(Gene) %>% tail()
...
for(i in 1:N_Nodes){
  ...
  #FilePath = paste("Data/Retrieved/Genes_and_diseases/Ukbb/a", Node, ".imp.stats", sep = "")
  
  FilePath = paste("E:/UK_Biobank2/Data/Retrieved/Ukbb/bolt/a", Node, ".imp.stats", sep = "")
...
# FILTERING OUT MHC
MhcGenArr = readRDS("Data/Retrieved/Genes_and_diseases/Ranges/MhcGeneArray.rds")
...
saveRDS(ArdAgeFrm,"Data/Retrieved/Genes_and_diseases/Diseases/GeneFrame_ARDcode_GenAge.rds")

# CREATE ARD - ARC FRAME BASED ON THE HIERARCHY
HrcFrm = read.csv("Data/Retrieved/Genes_and_diseases/Diseases/Ukb_DiseaseRoots.csv")
HrcFrm$X = NULL
...
saveRDS(GenArdArcFrm,"Data/Retrieved/Genes_and_diseases/Diseases/GeneFrame_ARD_ARC.rds")
...
#summary2genes_intervals(sgwasRes, Node, Name, thr = 10000, GenesFrame, genes.granges)
...
summary2genes_intervals <- function(sgwasRes, Node, Name, thr, GenesFrame, genes.granges){
  ...
  if (significants > 0){
    ...
    gwas_as_GR <- gwas2GRanges(sgwasRes, SNP = 'SNP',start = 'BP',chr = 'CHR',genome = 'hg38')
