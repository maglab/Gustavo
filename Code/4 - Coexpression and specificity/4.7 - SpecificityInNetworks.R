...
TauFrm = read.csv("Data/Retrieved/Specificity/Tau_gene_V8.csv") 
...
DirIndIntFrm_PPI = read.csv('Data/Generated/Networks_and_predictions/Networks/PPI/Proximity_and_Distances//Pleiotropy_and_IndirectInteractors.csv') 
...
# COX90 ARC-Interactions
IndFrm_COX90 = read.csv('Data/Generated/Networks_and_predictions/Networks/COX90/Proximity_and_Distances/Pleiotropy_and_IndirectInteractors.csv') %>%
  filter(iARC_Interactor>0) %>% select(Gen,iARC_Interactor) %>% rename(Num=iARC_Interactor) %>%
  mutate(TypInt = "Ind") %>% 
  mutate(NumThr = ifelse(Num>=4,"4+",Num)) %>% 
  mutate(Dts = "COX90") %>%
...
# COX95 ARC-Interactions
IndFrm_COX95 = read.csv('Data/Generated/Networks_and_predictions/Networks/COX95/Proximity_and_Distances/Pleiotropy_and_IndirectInteractors.csv')  %>%
  filter(iARC_Interactor>0) %>% select(Gen,iARC_Interactor) %>% rename(Num=iARC_Interactor) %>%
  mutate(TypInt = "Ind") %>% 
  mutate(NumThr = ifelse(Num>=4,"4+",Num)) %>% 
  mutate(Dts = "COX95") %>%
...
# KEGG ARC-Interactions
IndFrm_KEGG = read.csv('Data/Generated/Networks_and_predictions/Networks/KEGG/Proximity_and_Distances/Pleiotropy_and_IndirectInteractors.csv') %>%
  filter(iARC_Interactor>0) %>% select(Gen,iARC_Interactor) %>% rename(Num=iARC_Interactor) %>%
  mutate(TypInt = "Ind") %>% 
  mutate(NumThr = ifelse(Num>=4,"4+",Num)) %>% 
  mutate(Dts = "KEGG") %>%
...
for(z in 1:DtsLen){
  ...
  grouped_means <- TstFrm %>%
    group_by(NumThr) %>%
    summarise(
      N = sum(!is.na(Tau)),         # Número de observaciones no missing
      MeanTau = mean(Tau, na.rm = TRUE),
      Q25 = quantile(Tau, probs = 0.25, na.rm = TRUE), # Primer cuartil (Q25)
      Q75 = quantile(Tau, probs = 0.75, na.rm = TRUE), # Tercer cuartil (Q75)
      SD = sd(Tau, na.rm = TRUE),   # Desviación estándar (para el IC)
      SE = SD / sqrt(N),            # Error estándar de la media
      CI_lower = MeanTau - 1.96 * SE, # Límite inferior del IC del 95%
      CI_upper = MeanTau + 1.96 * SE  # Límite superior del IC del 95%
    )
...
MeanMedianFrm %>% filter(Dataset %in% "PPI") %>% select(ARC_Connected_Threshold,Mean_Tau,Median_Tau,Dataset)

write.csv(TauDirIndFrm,"Data/Generated/Specificity_and_coexpression/Specificity/Pleiotropies_Interactors_Scores.csv",row.names=FALSE)
write.csv(MeanMedianFrm,"Data/Generated/Specificity_and_coexpression/Specificity/Pleiotropies_Interactors_MeanMedian.csv",row.names=FALSE)
