...
CoxFrm = read.csv("Data/Generated/Specificity_and_coexpression/Coexpression/BoxPlot_Data.csv") %>%
...
TauFrm = read.csv("Data/Generated/Specificity_and_coexpression/Specificity/BoxPlot_Data.csv") %>%
  rename(Phn=Phenotype, Scr=Score, Lbl=Label) %>%
  filter(!(Phn %in% c("ARC-Pleiotropy_1","ARC-Pleiotropy_2","ARC-Pleiotropy_3",
                      "ARC-Pleiotropy_4","ARC-Pleiotropy_5","ARC-Pleiotropy_6")))
...
p <- ggplot(StsCoxTau, aes(x = Median.Tau, y = Median.Cox, label = Phn)) +
  ...
  theme_minimal() +
  labs(title = "Specificity vs Intra-set Coexpression in GenAge and ARCs",
       x = "Specificity", y = "Instra-set Coexpression") +
