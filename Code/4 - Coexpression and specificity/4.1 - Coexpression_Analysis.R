...
GenMat =  readRDS("Data/Retrieved/Network_Sources/Coexpression/Genes_and_edges/MatrixCox_AgeARD_Hgnc.rds")
...
IntPhnPhnFrm = NumPhnPhnFrm %>% filter(ColPhnSet == RowPhnSet) %>% select(Scr, ColPhnSet, ColPhnNum) %>% rename(Phn=ColPhnSet, Num=ColPhnNum)
...
HghFrm = DirIntFrm %>% 
  filter(Phn %in% c("ARC-Pleiotropy_4","ARC-Pleiotropy_5","ARC-Pleiotropy_6")) %>%
  mutate(Phn = "High ARC-Pleiotropy")
  
LowFrm = DirIntFrm %>% 
  filter(Phn %in% c("ARC-Pleiotropy_1","ARC-Pleiotropy_2","ARC-Pleiotropy_3")) %>%
  mutate(Phn = "Low ARC-Pleiotropy")  


GenPhnFrm = rbind(GenArcFrm,DirIntFrm,GenAgeFrm,HghFrm,LowFrm)
...
saveRDS(MatPhnPhnLst, "Data/Generated/Specificity_and_coexpression/Coexpression/Coexpression_Matrices_List.rds")  
saveRDS(PhnMeanFrm, "Data/Generated/Specificity_and_coexpression/Coexpression/Self_Coexpression_Mean_Frame.rds") 
saveRDS(PhnPhnScrFrm, "Data/Generated/Specificity_and_coexpression/Coexpression/Coexpression_Pairwise_Frame.rds")   
...
SumPhnFcn = function(GenSumPhn, GenMat){ 
  ...
  for(x in 1:SetLen){
    ...
    for(y in 1:SetLen){
      ...
      ssPhnPhnFrm = data.frame(Scr = PhnPhn.Arr, ColPhnSet = ColQry, RowPhnSet = RowQry)
