...
TmpPth = "Data/Generated/Permutations/Network_Associations/PermutationTable_ARC_Distance_10000.csv"

combined_summary = read.csv(TmpPth)

combined_summary = combined_summary %>% filter(Balance_Type != "Balanced_by_Degree")
...
combined_summary <- combined_summary %>%
  mutate(
    Group = factor(Group, levels = c(
      "AgeGen", "ModGen", "DisGen", "HgnGen", "DisNgb"
    ))
  ) %>%
  arrange(Network, Group) 


combined_summary$Group[combined_summary$Group=="AgeGen"] = "GenAge.Hum"
combined_summary$Group[combined_summary$Group=="ModGen"] = "GenAge.Mod"
combined_summary$Group[combined_summary$Group=="DisGen"] = "Diseases"
combined_summary$Group[combined_summary$Group=="HghGen"] = "High ARC-Pleiotropy"
combined_summary$Group[combined_summary$Group=="DisNgb"] = "Neighbhours"


write.csv(combined_summary, TmpPth, row.names = FALSE)
